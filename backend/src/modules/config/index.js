...
const createConfigModule = ({ sharedRuntime } = {}) => {
  ...
  const useCases = {
    ...
    listRatePolicies: createListRatePolicies({ configRepository }),
    createRatePolicy: createCreateRatePolicy({ configRepository }),
    updateRatePolicy: createUpdateRatePolicy({ configRepository }),
    deleteRatePolicy: createDeleteRatePolicy({ configRepository }),
    resolveRatePolicy: createResolveRatePolicy({ configRepository }),
    ...
    resolveLateFeePolicy: createResolveLateFeePolicy({ configRepository }),
