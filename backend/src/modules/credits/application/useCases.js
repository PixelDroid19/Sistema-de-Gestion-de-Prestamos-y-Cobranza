...
const buildPromiseToPayPdfBuffer = ({ promise, loan, customer }) => {
  ...
  return buildReportPdf({
    ...
    subtitle: `Documento #${promise.id} · Generado el ${formatDateOnlyForDocument(promise.createdAt)}`,
    summary: [
      { label: 'Crédito', value: `#${loan.id}` },
      { label: 'Monto del crédito', value: formatCurrencyForDocument(loan.amount) },
      { label: 'Cliente', value: customerName },
      { label: 'Correo del cliente', value: customerEmail },
      { label: 'Fecha prometida', value: formatDateOnlyForDocument(promise.promisedDate) },
      { label: 'Monto prometido', value: formatCurrencyForDocument(promise.amount) },
      { label: 'Estado', value: formatPromiseStatus(promise.status) },
      ...(promise.notes ? [{ label: 'Notas', value: promise.notes }] : []),
    ],
    sections: [
      {
        heading: 'Condiciones',
...
const createDownloadPromiseToPay = ({ promiseRepository, loanAccessPolicy }) => async ({ actor, loanId, promiseId }) => {
  ...
  return {
    ...
    buffer: await buildPromiseToPayPdfBuffer({ promise, loan, customer }),
  };
};
