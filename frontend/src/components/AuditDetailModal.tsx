...
const detailTabs: DetailTab[] = ['resumen', 'cambios', 'metadatos'];

const getTabLabel = (tab: DetailTab) => {
  switch (tab) {
    case 'cambios':
      return tTerm('audit.detail.tabs.changes');
    case 'metadatos':
      return tTerm('audit.detail.tabs.metadata');
    default:
      return tTerm('audit.detail.tabs.summary');
  }
};
  ...
  return (
    <div className="min-w-0">
      ...
      <div className={`mt-1 truncate text-sm font-semibold text-text-primary ${mono ? 'font-mono' : ''}`} title={typeof value === 'string' ? value : undefined}>
        {value || tTerm('audit.detail.notRecorded')}
      </div>
    </div>
  );
}
...
function JsonPanel({ title, data }: { title: string; data: unknown }) {
  const formatted = hasData(data) ? JSON.stringify(data, null, 2) : tTerm('audit.detail.noData');
...
export default function AuditDetailModal({ auditLog, onClose }: AuditDetailModalProps) {
  ...
  return (
    <ModalShell
      title={tTerm('audit.detail.title')}
      subtitle={tTerm('audit.detail.subtitle')}
      maxWidthClassName="max-w-4xl"
      onClose={onClose}
      footer={<ActionButton onClick={onClose}>{tTerm('audit.detail.close')}</ActionButton>}
    >
        <div className="overflow-y-auto">
          <p className="mb-4 text-[10px] font-bold uppercase tracking-[0.18em] text-text-secondary">{tTerm('audit.detail.eyebrow')}</p>
          <section className="grid gap-3 border-b border-border-subtle pb-4 md:grid-cols-3">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <Fact label={tTerm('audit.detail.service')} value={service} mono />
            </div>
            <div className="flex min-w-0 items-center gap-3">
              ...
              <Fact label={tTerm('audit.detail.sourceIp')} value={auditLog.ip || tTerm('audit.detail.notRecorded')} mono />
            </div>
            <div className="flex min-w-0 items-center gap-3">
              ...
              <Fact
                label={tTerm('audit.detail.action')}
                value={(
          ...
          <ViewTabs
            className="mt-4"
            ariaLabel={tTerm('audit.detail.tabs.aria')}
            activeTab={activeTab}
            onChange={(tabId) => setActiveTab(tabId as DetailTab)}
            tabs={tabItems.map((tab) => ({ id: tab, label: getTabLabel(tab) }))}
          />

          {activeTab === 'resumen' && (
            <section className="grid gap-x-6 gap-y-4 py-4 sm:grid-cols-2 lg:grid-cols-3">
              <Fact label={tTerm('audit.detail.event')} value={`#${auditLog.id}`} mono />
              <Fact label={tTerm('audit.detail.date')} value={formatAuditDate(auditLog.timestamp)} />
              <Fact label={tTerm('audit.detail.user')} value={auditLog.userName || tTerm('audit.table.user.system')} />
              <Fact label={tTerm('audit.detail.userNumber')} value={auditLog.userId || tTerm('audit.detail.noSession')} mono />
              <Fact label={tTerm('audit.detail.area')} value={getAuditModuleLabel(auditLog.module)} />
              <Fact label={tTerm('audit.detail.entity')} value={getAuditEntityTypeLabel(auditLog.entityType)} />
              <Fact label={tTerm('audit.detail.entityNumber')} value={auditLog.entityId || tTerm('audit.detail.noNumber')} mono />
              <Fact label={tTerm('audit.detail.httpMethod')} value={method || tTerm('audit.detail.notRecorded')} mono />
              <Fact label={tTerm('audit.detail.path')} value={path || tTerm('audit.detail.notRecorded')} mono />
            </section>
          )}

          {activeTab === 'cambios' && (
            <section className="grid gap-4 py-4 lg:grid-cols-2">
              <JsonPanel title={tTerm('audit.detail.previousData')} data={auditLog.previousData} />
              <JsonPanel title={tTerm('audit.detail.newData')} data={auditLog.newData} />
            </section>
          )}
            ...
            <section className="grid gap-4 py-4 lg:grid-cols-[0.85fr_1.15fr]">
              <div className="space-y-4">
                <div className="flex items-start gap-3 rounded-xl border border-border-subtle bg-bg-base p-3">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-text-primary">{tTerm('audit.detail.httpClient')}</p>
                    <p className="mt-1 break-words font-mono text-xs leading-5 text-text-secondary">
                      {auditLog.userAgent || tTerm('audit.detail.userAgentMissing')}
                    </p>
                  </div>
                </div>
                <div className="flex items-start gap-3 rounded-xl border border-border-subtle bg-bg-base p-3">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-text-primary">{tTerm('audit.detail.serviceContext')}</p>
                    <p className="mt-1 break-words font-mono text-xs leading-5 text-text-secondary">
                      {[method, path].filter(Boolean).join(' ') || tTerm('audit.detail.serviceContextMissing')}
                    </p>
                  </div>
                </div>
              </div>
              <JsonPanel title={tTerm('audit.detail.technicalMetadata')} data={auditLog.metadata} />
            </section>
          )}
        </div>
    </ModalShell>
